...
export function Login({ onLogin, onNavigateToRegister }: LoginProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setTimeout(() => {
      const valid = ADMIN_ACCOUNTS.some(
        acc => acc.username === username.trim() && acc.password === password
      );
      if (valid) {
        // Lưu session đơn giản vào sessionStorage
        sessionStorage.setItem('pv_auth', JSON.stringify({ username: username.trim(), role: 'admin' }));
        onLogin();
      } else {
        setError('Sai Operator ID hoặc mật khẩu. Thử lại.');
      }
      setIsLoading(false);
    }, 1200);
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', bottom: '15%', right: '-15%', width: 380, height: 380, background: 'rgba(14,165,233,.04)', borderRadius: '50%', filter: 'blur(80px)', pointerEvents: 'none' }} />

      <div style={{ width: '100%', maxWidth: 440, position: 'relative', zIndex: 1, animation: 'fadeUp .35s ease' }}>
        {/* Brand header */}
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{
            ...
            borderRadius: 16,
            marginBottom: 16,
            boxShadow: '0 0 32px rgba(14,165,233,.15)',
          }}>
          ...
          <p style={{
            ...
            marginTop: 6,
          }}>Security & Recognition Protocol</p>
        </div>
        ...
        <div style={{
          ...
          <form onSubmit={handleSubmit} style={{ display: 'grid', gap: 20 }}>
            ...
            <label style={{ display: 'grid', gap: 7 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <button type="button" style={{ fontSize: 10, fontWeight: 700, color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '.06em' }}>
                  Quên Pass?
                </button>
              </div>
              <div style={{ position: 'relative' }}>
                ...
                <button
                  ...
                  style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', display: 'flex', padding: 4 }}
                >
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
              <input type="checkbox" style={{ width: 14, height: 14, accentColor: 'var(--accent)', padding: 0 }} />
            ...
            <button
              ...
              style={{ justifyContent: 'center', padding: '14px 20px', fontSize: 12, letterSpacing: '.15em', marginTop: 4 }}
            >
              {isLoading ? (
          ...
          <div style={{ marginTop: 28, paddingTop: 20, borderTop: '1px solid var(--border)', textAlign: 'center' }}>
            ...
            <button
              ...
              style={{ fontSize: 11, fontWeight: 900, color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '.1em' }}
            >
