...
import {
  Camera, Car, ChevronLeft, ChevronRight, History,
  LayoutDashboard, LogOut, ScanLine, Settings, UserCircle, Users,
} from 'lucide-react';
...
const ADMIN_MENU: { id: ViewType; label: string; icon: typeof LayoutDashboard }[] = [
...
// USER: không có Dashboard, không có Chủ xe, không có Cấu hình
const USER_MENU: { id: ViewType; label: string; icon: typeof LayoutDashboard }[] = [
  { id: 'my-profile',  label: 'Hồ sơ của tôi',  icon: UserCircle },
  { id: 'recognition', label: 'Nhận diện ảnh',   icon: ScanLine },
  { id: 'camera',      label: 'Camera realtime',  icon: Camera },
  { id: 'history',     label: 'Lịch sử xe tôi',  icon: History },
];
  ...
  return (
    <aside className={`sidebar${collapsed ? ' collapsed' : ''}`}>
      ...
      <a
        ...
        onClick={e => {
          e.preventDefault();
          setActiveView(isAdmin ? 'dashboard' : 'my-profile');
        }}
      >
